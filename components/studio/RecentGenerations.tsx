  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2.5 md:grid-cols-3 lg:grid-cols-6">
        {recent.map((item) => {
          const hasDeliverable = Boolean(item.output_url);
          return (
            <Link
              key={item.job_id}
              href="/studio/history"
            ...
            >
              ...
              <div className="absolute right-1.5 top-1.5">
                {item.status === 'done' && hasDeliverable ? (
                  ...
                  <Clock size={13} className="text-accent-yellow drop-shadow" />
                ) : (
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/80 via-black/30 to-transparent p-2">
                ...
                {item.duration_s != null && (
                  <div className="mt-0.5 text-[9px] text-white/60">
                    {item.duration_s}s - {hasDeliverable ? 'Autonomous' : 'Delivery pending'}
                  </div>
                )}
              </div>
            </Link>
