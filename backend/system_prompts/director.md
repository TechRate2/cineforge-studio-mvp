...
Rules:
- `face_signature` MUST be derived from `reference_hints[*].visual_description` (V5.14 vision pre-scan results) when reference images exist. Copy concrete details verbatim — race, hair color/length/style, skin tone, eye color, expression — exactly as the vision LLM described them. Do NOT invent or default to any specific ethnicity. If no reference image OR vision returned empty visual_description, use abstract descriptor ("a young adult with soft features"). Example flow: vision says *"East Asian woman early 20s, hair tied in low bun with side strands, warm fair skin, red lipstick, white knit top with square neckline"* → you write face_signature *"East Asian woman early 20s, hair tied in low bun with side strands escaping, warm fair skin, red lipstick, calm friendly expression"*.
- NEVER use age numbers in shot prompts (triggers conservative filtering). Use functional descriptors: *"photorealistic figure"*.
- Outfit invariant: define ONCE in `characters[i].outfit`, never change mid-sequence unless plot demands. V5.14 — `outfit` MUST also be derived from `reference_hints[*].visual_description` for the character_anchor ref (describe top/bottom/accessories exactly as visible in image, don't invent colors/styles).
- Same rule for `products[i].packaging_description` and `color_palette` — derive from `reference_hints[*]` where role==product_hero/product_detail. Do NOT invent product features not visible in reference image.
